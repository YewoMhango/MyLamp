...
(async function () {
    ...
    for (let child of Array.from(overlaySection.children)) {
        (child as HTMLDivElement).onclick = (event) => event.stopPropagation();
    }
...
let verse_being_selected: Verse;
...
function selectChapter(chapter: number) {
    ...
    let numberOfVerses =
        verse_count_stats[bookNames.indexOf(verse_being_selected.book)][
            Number(chapter) - 1
        ];
...
async function search() {
    ...
    ipcRenderer.send(
        "request",
        `search/${searchType}/${language}/${searchText}`
    );
}
